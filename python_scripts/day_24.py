...
def dfs(start_pos, end_pos, start_time=0):
    ...
    while queue:
        ...
        current_blizzards = _cache.get(_time+1)
        if current_blizzards is None:
            current_blizzards = Blizzard.get_blizzards(_time+1)
            _cache[_time+1] = current_blizzards
            
        for move in move_directions:
            new_pos = (current_pos[0] + move[0], current_pos[1] + move[1])
            if new_pos in current_blizzards:
                continue
            if _map.get(new_pos) == "#" or _map.get(new_pos) is None:
                continue
            queue.append((new_pos, _time+1, bm+[(current_pos, new_pos, _time)]))
...
def solve(puzzle_input):
    ...
    best_time, best_moves = dfs(start_pos, end_pos, start_time=0)
    print(f"Part 1: {best_time} in {time.time() - start_1_time:.4f} seconds!")
    
    # Part 2
    start_2_time = time.time()
    best_moves_2 = best_moves
    bm = best_moves
    best_time_2 = best_time
    for _ in range(2):
        best_time_2, bm = dfs(bm[-1][1], bm[0][0], start_time=best_time_2)
        best_moves_2.extend(bm)
