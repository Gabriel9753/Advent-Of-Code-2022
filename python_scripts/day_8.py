...
def search_part_1(grid, iter_x, iter_y, vertical=True):
    global VISIBLES
    for x in iter_x:
        current_tallest = -1
        for y in iter_y:
...
def solve(puzzle_input):
    ...
    grid_y = len(grid)
    start = time()
    search_part_1(grid, range(grid_x), range(grid_y))
    search_part_1(grid, range(grid_x), range(grid_y-1, -1, -1))
    search_part_1(grid, range(grid_y), range(grid_x), vertical=False)
    search_part_1(grid, range(grid_y), range(grid_x-1, -1, -1), vertical=False)
    logger.info(f"Solution Part 1: {len(VISIBLES)}")
    print(f"Time taken: {time() - start:.15f}s")
    start = time()
